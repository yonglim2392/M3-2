...
for index, order in orders_df.iterrows():
    ...
    while remaining_qty > 0:
        ...
        qty_this_segment = 0 # Quantity produced in this segment
        if remaining_qty <= qty_producible_today: # 오늘 안에 완료 가능
            ...
            qty_this_segment = remaining_qty # Capture quantity for this segment
            remaining_qty = 0
        else: # 오늘 안에 완료 불가능, 다음 날로 이월
            segment_finish_time = datetime.combine(current_day_date, time(WORK_END_HOUR, 0))
            qty_this_segment = qty_producible_today # Capture quantity for this segment
        ...
        segments.append({
            ...
            'qty_produced': qty_this_segment # Add quantity produced in this segment
        })
    ...
    delay_days_overall = (final_finish_time.date() - delivery_date.date()).days if final_finish_time.date() > delivery_date.date() else 0
    ...
    for segment in segments:
        scheduled_tasks.append({
            'PROD_NO': prod_no,
            'Style': style,
            'Q\'TY': segment['qty_produced'], # 세그먼트별 생산 수량
